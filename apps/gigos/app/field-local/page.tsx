...
export default async function FieldLocalPage({ searchParams }: { searchParams: any }) {
  const mode = searchParams.mode??'WALK', speed=SPEEDS[mode]??3
  const where: any = { domainType:'FIELD_LOCAL', status:'AVAILABLE' }
  if(searchParams.minPayout) where.payoutAmount={gte:parseFloat(searchParams.minPayout)}
  if(searchParams.maxMinutes) where.estMinutes={lte:parseInt(searchParams.maxMinutes)}
  let raw: any[] = []
  try { raw = await prisma.task.findMany({ where, include:{provider:true}, take:100 }) } catch {}
  const maxDist = searchParams.maxDistance ? parseFloat(searchParams.maxDistance) : null
  let tasks = raw.map(t => {
  ...
  const s = searchParams.sortBy??'deadline'
  ...
  return (
    <main className="space-y-4">
      ...
      <Suspense><FieldLocalFilters initial={searchParams}/></Suspense>
